...
const StripClub = require("./stripclub.js");
...
class Routes {
    ...
    static initializeRoutes(fastify) {
        ...
        StripClub.talkToHooker(fastify);
        ...
        fastify.all("/mode/offline", async function (request, reply) {
            reply
                .send(reply
                    .resNoBody(database.core.serverConfig.Patches));
        });

        fastify.all("/mode/offlineNodes", async function (request, reply) {
            reply
                .send(reply
                    .resNoBody(database.core.serverConfig.PatchNodes));
        });
        ...
        fastify.all("/client/game/start", async function (request, reply) {
            ...
            if (account.clientHasAccount(request.cookies.PHPSESSID)) {
                reply
                    .type('application/json')
                    .resBSG({ utc_time: Date.now() / 1000 }, 0, null, true)
                return
            }
            reply
                .type('application/json')
                .resBSG({ utc_time: Date.now() / 1000 }, 999, "Profile not found", true)
        });

        fastify.all("/client/menu/locale/:lang", async function (request, reply) {
            reply
                .send(reply
                    .resBSG(language.getMenu(request.params.lang, database.locales)));
        });
